...
def five(file):
    ...
    for i in range(len(first)):
        if first[i][0]==second[i][0]:
            ...
            if first[i][0]>second[i][0] and first[i][1]>second[i][1]:
                x = second[i][0]
                y = second[i][1]
                for i in range(first[i][0]-second[i][0]+1):
                    arr[y][x]+=1
                    x+=1
                    y+=1

            elif first[i][0]<second[i][0] and first[i][1]<second[i][1]:
                x = first[i][0]
                y = first[i][1]
                for i in range(second[i][0]-first[i][0]+1):
                    arr[y][x]+=1
                    x+=1
                    y+=1
            elif first[i][0]>second[i][0] and first[i][1]<second[i][1]:  
